...
export class SubcategoryService extends BaseService {
  ...
  private toPayload(input: CreateSubcategoryInput | UpdateSubcategoryInput): SubcategoryPayload {
  ...
  async create(input: CreateSubcategoryInput) {
    ...
    const existing = await this.repo.findByCategoryAndName(input.categoryId, input.name.trim());
    if (existing) this.throwBadRequest("Subcategory name already exists for this category");

    const created = await this.repo.create(this.toPayload(input) as Required<SubcategoryPayload>);
  ...
  async update(id: string, input: UpdateSubcategoryInput) {
    ...
    const duplicate = await this.repo.findByCategoryAndName(nextCategoryId, nextName);
