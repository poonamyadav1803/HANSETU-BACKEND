...
export class CategoryService extends BaseService {
  ...
  private normalizeSubcategories(subcategories?: string[]) {
    ...
    return Array.from(new Set(subcategories.map((name) => name.trim()).filter(Boolean)));
  }

  private toCategoryPayload(input: CreateCategoryInput | UpdateCategoryInput): CategoryPayload {
    ...
    if ("primaryColor" in input) payload.primaryColor = input.primaryColor ?? null;
    if ("secondaryColor" in input) payload.secondaryColor = input.secondaryColor ?? null;
    if ("gradientColor" in input) payload.gradientColor = input.gradientColor ?? null;
    if ("badgeColor" in input) payload.badgeColor = input.badgeColor ?? null;
    if ("isActive" in input && input.isActive !== undefined) payload.isActive = input.isActive;
  ...
  async update(id: string, input: UpdateCategoryInput) {
    ...
    if (input.slug && input.slug.trim() !== existingCategory.slug) {
      const categoryWithSlug = await this.repo.findPlainBySlug(input.slug.trim());
    ...
    const updatedCategory = await this.repo.update(id, categoryPayload, subcategoryNames);
