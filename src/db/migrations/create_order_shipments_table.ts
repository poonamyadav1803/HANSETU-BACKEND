/**
 * Migration: Create order_shipments table for the legacy `orders`-based order flow.
 *
 * Run with: npx ts-node-dev --transpile-only src/db/migrations/create_order_shipments_table.ts
 */
...
async function migrate() {
  log("Running migration: create_order_shipments_table");
  ...
  try {
    ...
    log("  ✓ order_shipments table ready");
